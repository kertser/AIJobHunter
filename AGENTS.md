...
- **linkedin/** — Playwright browser automation: `session.py` (cookie auth + `remote_login()` for headless/Docker programmatic login with verification code support and checkpoint screenshot streaming), `discover.py` (search + pagination), `parse.py` (HTML→data), `apply.py` (Easy Apply wizard), `forms.py`/`form_filler_llm.py` (LLM-powered form filling), `selectors.py` (CSS/XPath constants)
...
- **market/title_normalizer.py** — `TitleNormalizer` base + `HeuristicTitleNormalizer` (regex + fuzzy clustering via RapidFuzz) + `OpenAITitleNormalizer` (LLM batch normalisation) + `FakeTitleNormalizer` (deterministic, for tests); plugged into `build_role_archetypes()` and `run_market_pipeline()`
...
- **web/templates/market.html** — added 🏢 Top Companies panel between Rising Trends and Role Archetypes; added gap analysis and dialogue API links to the endpoints list
...
- **market/pipeline.py** — `run_market_pipeline(session, extractor, profile, candidate_key, title_normalizer)` chains all 7 market steps sequentially (ingest → extract → graph → trends → role-model → candidate-model → match); commits after each step for partial progress; returns summary dict with counts; shared by CLI and web
- **market/cli.py** — added `hunt market run-all --extractor heuristic|openai|fake --normalizer heuristic|openai|fake|legacy --profile <name>` single-command full pipeline execution; loads user profile from `user_profile.yml` when available
- **web/routers/run.py** — added `POST /api/run/market` endpoint; runs `run_market_pipeline()` as a background task via `TaskManager` with SSE progress; auto-selects extractor and title normaliser (fake in mock mode, OpenAI if key available, heuristic otherwise); loads user profile for candidate model
...
- **Authentication**: JWT-based sessions via `access_token` cookie (7-day expiry). `login_required_middleware` redirects unauthenticated page requests to `/login` and returns 401 for API calls. Public paths: `/login`, `/register`, `/api/auth/`, `/api/health`, `/static/`, `/favicon.ico`, `/api/settings/cookies-paste` (public for browser extension). Admin panel protected by a separate `admin_token` cookie (1-hour, standalone password gate via `require_admin`). CORS middleware allows `chrome-extension://` and `localhost` origins for the extension's cross-origin cookie-paste requests.
