...
- **linkedin/** — Playwright browser automation: `session.py` (cookie auth + `remote_login()` for headless/Docker programmatic login with verification code support), `discover.py` (search + pagination), `parse.py` (HTML→data), `apply.py` (Easy Apply wizard), `forms.py`/`form_filler_llm.py` (LLM-powered form filling), `selectors.py` (CSS/XPath constants)
